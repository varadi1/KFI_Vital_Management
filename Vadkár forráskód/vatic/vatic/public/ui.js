...
function ui_setup(job)
{
    ...
    $("<table>" + 
        "<tr>" +
            "<td><div id='instructionsbutton' class='button'>Instructions</div><div id='instructions'>Kérjük jelöljön be minden objektumot a videón.</td>" +
    ...
    $("#bottombar").append("<div class='button' id='rewindbutton'>Elejére</div> ");
    $("#bottombar").append("<div class='button' id='playbutton'>Lejátszás</div> ");
    $("#bottombar").append("<div class='button' id='exportbutton'>Export</div> ");
    $("#topbar").append("<div id='newobjectcontainer'>" +
        "<div class='button' id='newobjectbutton'>Új objektum</div></div>");

    $("<div id='objectcontainer'></div>").appendTo("#sidebar");

    $("<div class='button' id='openadvancedoptions'>Beállítások</div>")
        .button({
            icons: {
...
function ui_setupbuttons(job, player, tracks)
{
    ...
    $("#exportbutton").click(function() {
        if (!$(this).button("option", "disabled"))
        {
            ...
            var text=""
            for (fr = job.start; fr < job.stop; fr++) {  
                //console.log(fr);
                text += "Frame: " + fr + "\n";
                for (t in tracks.tracks) {
                    text += "Track: " + t + "\n";
                    //console.log(tracks.tracks[t]);
                    text += "Type: " + job.labels[tracks.tracks[t].label] + "\n";
                    var pos=tracks.tracks[t].estimate(fr);
                    text += "Annotation: " + JSON.stringify(pos) + "\n";
                    console.log(pos);
                }
             } 
   ...
   var data = text, fileName = 'annotation_'+job.jobid+'.txt';

saveData(data, fileName);

    //this.track.estimate(this.player.frame)
        }
    }).button({
        disabled: false,
        icons: {
...
$(window).keydown(function(e) {
	var keycode = e.keyCode ? e.keyCode : e.which;
        if  ((keycode == '115' || keycode == '83' ) && (e.ctrlKey || e.metaKey) && !(e.altKey)){
...
function ui_submit(job, tracks)
{
    ...
    function savejob(callback)
    {
        ...
        server_post("savejob", [job.jobid],tracks.serialize(),
           function(data) {
               callback()
           });
    }
