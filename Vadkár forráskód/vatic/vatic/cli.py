...
class visualize(DumpCommand):
    ...
    def __call__(self, args):
        ...
        vision.visualize.save(it,
            lambda x: "{0}/{1}.png".format(args.output, x))
...
class dump(DumpCommand):
    ...
    def dumppascal(self, folder, video, data, difficultthresh, skip,
        ...
        for frame in allframes:
            ...
            file.write("<annotation>")
            file.write("<folder>{0}</folder>".format(folder))
            file.write("<filename>{0}.png</filename>".format(strframe))
        ...
        for label, frames in hasit.items():
            ...
            if pascalds:
                ...
                for neg in negs:
                    ...
                    tree.find("filename").text = "n{0}.png".format(neg)
                    try:
                        os.makedirs(os.path.dirname("{0}/Annotations/n{1}.xml".format(folder, neg)))
                    except OSError:
                        pass
                    try:
                        os.makedirs(os.path.dirname("{0}/JPEGImages/n{1}.png".format(folder, neg)))
                    except OSError:
                        pass
                    tree.write("{0}/Annotations/n{1}.xml".format(folder, neg))
                    shutil.copyfile("{0}/JPEGImages/{1}.png".format(negdir, neg),
                                    "{0}/JPEGImages/n{1}.png".format(folder, neg))
        ...
        for frame in allframes:
            ...
            dest = "{0}/JPEGImages/{1}.png".format(folder, strframe)
            try:
                os.unlink(dest)
            except OSError:
                pass
    ...
    def __call__(self, args):
        ...
        for worker in workers:
            ...
            for job in jobs:
                ...
                paths = [x.getboxesonly(interpolate = True,
                                    bind = True,
                ...
                image.save("{0}/{1}-{2}.png".format(args.directory,
                                                    worker.id,
                                                    job.hitid))
