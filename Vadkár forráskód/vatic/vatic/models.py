...
class Path(turkic.database.Base):
	...
	for box in self.boxes:
		result.append(box.getbox())
		pos=[]
		for p in box.polygons:
			pos.append(p)
		pos.sort(key = lambda x: x.number)
		polygons.append(pos)

	both = zip(result, polygons)
	both.sort(key = lambda x: x[0].frame)
	

	result= [i[0] for i in both] 
	sortedpolygons= [i[1] for i in both] 
	...
	#with open('/home/horan/vatic_install/vatic/log.txt', 'a') as the_file:
    	#		the_file.write(str(both))
        return result, sortedpolygons

    def getboxesonly(self, interpolate = False, bind = False, label = False):
	result = []
	polygons = []
	for box in self.boxes:
		result.append(box.getbox())
...
class Polygon(turkic.database.Base):
    __tablename__ = "polygons"

    id = Column(Integer, primary_key = True)
    boxid = Column(Integer, ForeignKey(Box.id))
    box = relationship(Box,
                        backref = backref("polygons", cascade = "all,delete"))
